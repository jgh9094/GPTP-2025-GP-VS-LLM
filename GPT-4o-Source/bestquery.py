...
def main():
    ...
    loop_through_datasets(dataset_names=test_names, query=empty_query, portion='200', iterations=20)
...
def loop_through_datasets(dataset_names, query, iterations, portion, API=AzureAPIHandler):
    ...
    #print(my_dict)
    total_vals = []
    for j in ['chen_returns', 'chen_equals','shalinbars','sharlincsv','austin']:
        for name in dataset_names:
            train_path = f"datasets/{name}/{name}_train.csv"
            val_path = f"datasets/{name}/{name}_val.csv"
            traincount = 0
            testcount = 0
            success_rates = []
            ...
            with open(f'datasets/{name}/{name}_{j}_per_dict.csv', 'w') as csv_file:  
        ...
        print(results_dict)
        with open(f'datasets/{j}_result.csv', 'w') as csv_file:  
            writer = csv.writer(csv_file)
