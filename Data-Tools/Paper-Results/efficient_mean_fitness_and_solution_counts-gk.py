...
def print_stats(outputDirectory, verbose=True):
    ...
    #    f = open(outputDirectory + fileName)
        ...
        for line in reverse_readline(outputDirectory + fileName):
            ...
            if line.startswith("program:") and first_val == 0:
    ...
    if csv:
        ...
        for i in range(len(list_of_functions)):
            if i in list_of_successful:
                success_ful.append(1)
            else:
                success_ful.append(0)

        #with open(f"{outputDirectory}my_dict.csv", 'w') as file:
        #    writer = csv.writer(file)
        #    writer.writerows(zip(list_of_functions, success_ful))

        my_dict = dict(enumerate(list_of_functions))
        success_dict = dict(enumerate(success_ful))
        #print(my_dict) 

        directory_info = outputDirectory.split("/")
        current_file = directory_info[-2]
        print(current_file)
        size_file = ""
        for i in directory_info[:-2]:
            if size_file == "":
                size_file += i
            else:
                size_file += "/" + i
        ...
        if not os.path.exists(f"{size_file}outputs/{current_file}"): 
            os.makedirs(f"{size_file}outputs/{current_file}") 

        with open(f"{size_file}outputs/{current_file}/my_dict.json", "w") as file:
            json.dump(my_dict, file)
        
        with open(f"{size_file}outputs/{current_file}/my_success.json", "w") as file:
